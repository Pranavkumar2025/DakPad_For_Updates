...
const UserDashboard = () => {
  ...
  const handleApplicationIdSearch = async () => {
    ...
    try {
      const res = await api.get(`/api/applications/${applicationIdInput.trim()}`); // <-- sends cookie
      const app = res.data;
        ...
        timeline: timeline.length > 0 ? timeline : [
          {
            ...
            comment: `Received at ${app.block || "N/A"} on ${formatDate(app.applicationDate)}`,
      ...
      setError(err.response?.data?.error || err.message || "Application not found");
  ...
  const getStatusStyle = (status) => {
    switch (status) {
      ...
      default:
        return { bg: "bg-blue-100", text: "text-blue-700", badge: "bg-blue-500 text-white", icon: <Clock size={20} /> };
    }
  };
          ...
          <div className="max-w-4xl mx-auto text-center">
            ...
            >
              <h1 className="text-5xl md:text-6xl font-bold mb-4 font-['Montserrat'] flex justify-center gap-2">
                ...
                <motion.span className="text-transparent bg-clip-text bg-gradient-to-r from-[#ff5010] to-[#fc641c]">
                  {isHindi
                    ? t("dashboard.application")
                    : Array.from(t("dashboard.application")).map((letter, i) => (
                        <motion.span
                          key={i}
            ...
            >
              {[
                { icon: Clock, title: t("dashboard.features.realTime.title"), desc: t("dashboard.features.realTime.desc") },
                { icon: QrCode, title: t("dashboard.features.qrCode.title"), desc: t("dashboard.features.qrCode.desc") },
                { icon: Download, title: t("dashboard.features.download.title"), desc: t("dashboard.features.download.desc") },
                { icon: CheckCircle, title: t("dashboard.features.secure.title"), desc: t("dashboard.features.secure.desc") },
              ].map((f, i) => (
                <motion.div key={i} whileHover={{ y: -5 }} className="text-center p-6">
                  ...
                  <h3 className="font-semibold text-gray-800 mb-2 font-['Montserrat']">{f.title}</h3>
                  <p className="text-gray-600 text-sm font-['Montserrat']">{f.desc}</p>
                </motion.div>
              ))}
            </motion.div>
          </div>
              ...
              {isModalLoading ? (
                ...
                <>
                  ...
                  <div className="space-y-8">
                    ...
                    >
                      ...
                      <div>
                        <div className="flex items-center gap-3">
                          ...
                          <span className={`px-3 py-1 rounded-full text-xs font-semibold ${getStatusStyle(foundApplication.status).badge}`}>
                            {foundApplication.status}
                          </span>
                        </div>
                        <p className={`text-sm ${getStatusStyle(foundApplication.status).text} mt-1 font-['Montserrat']`}>
                          {foundApplication.status === "Pending" && t("dashboard.statusMessages.pending")}
                          {foundApplication.status === "Compliance" && t("dashboard.statusMessages.compliance")}
                          {foundApplication.status === "Dismissed" && t("dashboard.statusMessages.dismissed")}
                        </p>
                        <p className="text-xs text-gray-500 mt-1 font-['Montserrat']">
                          {t("dashboard.modal.lastUpdated")} {foundApplication.lastUpdated}
                        </p>
                      </div>
                    </motion.div>

                    {/* Details */}
                    <motion.div className="bg-white rounded-xl p-6 shadow-md border border-gray-100">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 text-sm text-gray-700">
                        {[
                          { label: t("dashboard.modal.id"), value: foundApplication.applicationId, icon: <FileText size={18} className="text-green-600 mt-0.5" /> },
                          { label: t("dashboard.modal.name"), value: foundApplication.applicantName, icon: <User size={18} className="text-green-600 mt-0.5" /> },
                          { label: t("dashboard.modal.date"), value: foundApplication.dateOfApplication, icon: <Calendar size={18} className="text-green-600 mt-0.5" /> },
                          { label: t("dashboard.modal.subject"), value: foundApplication.subject, icon: <FileText size={18} className="text-green-600 mt-0.5" /> },
                          { label: t("dashboard.modal.description"), value: foundApplication.description, icon: <FileText size={18} className="text-green-600 mt-0.5" />, colSpan: true },
                        ].map((item, i) => (
                          <motion.div
                            key={i}
                        ...
                        <div className="flex flex-col sm:flex-row items-center gap-6">
                          ...
                          <div className="flex-1 text-center sm:text-left">
                            ...
                            <button
                              ...
                              className="inline-flex items-center px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 text-sm font-medium font-['Montserrat'] gap-2"
                            >
                              <Download size={16} />
                      ...
                      <motion.div className="bg-white rounded-xl p-6 shadow-md border border-gray-100">
                        ...
                        <div className="relative pl-8">
                          ...
                          {foundApplication.timeline.map((step, i) => {
                            ...
                            const isCompleted = isLast || step.section.includes("Compliance") || step.section.includes("Disposed");
                            const dotClass = isCompleted ? "bg-green-600" : "bg-orange-500";
                            return (
                              <motion.div
                                key={i}
                                className="flex items-start gap-4 bg-gray-50 p-4 rounded-lg border border-gray-100 mb-3 relative hover:bg-gray-100"
                                initial={{ opacity: 0, x: -10 }}
                                animate={{ opacity: 1, x: 0 }}
                                transition={{ delay: i * 0.06 }}
                              >
                                <div className={`w-6 h-6 rounded-full ${dotClass} flex items-center justify-center shadow-md border-2 border-white`}>
                                  {isCompleted && <CheckCircle size={16} className="text-white" />}
                                </div>
                                <div className="flex-1">
                                  <div className="flex justify-between items-center mb-1">
                                    <p className="text-sm font-semibold text-gray-800 font-['Montserrat']">{step.section}</p>
                    ...
                    {/* Action Buttons */}
                    <motion.div className="flex justify-end gap-4 mt-6">
                      <motion.button
                        className="flex items-center gap-2 px-5 py-2.5 rounded-full font-semibold text-sm bg-gray-100 text-gray-700 hover:bg-gray-200 font-['Montserrat'] shadow-sm"
