...
const AddCaseForm = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await api.post("/api/applications", form); // <-- sends cookie
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="text" name="name" value={formData.name} onChange={handleInputChange} placeholder="Enter applicant name" className="w-full p-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#ff5010] focus:border-transparent transition-all duration-200" />
              {errors.name && <p className="text-red-500 text-xs mt-1">{errors.name}</p>}
            </div>
            <div>
              <label className="text-xs font-medium text-gray-600">Application Date</label>
              <input type="date" name="applicationDate" value={formData.applicationDate} onChange={handleInputChange} className="w-full p-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#ff5010] focus:border-transparent transition-all duration-200" />
            ...
            <div>
              ...
              <div className="flex items-center border border-gray-300 rounded-lg focus-within:ring-2 focus-within:ring-[#ff5010] transition-all duration-200">
                ...
                <input type="text" name="phone" value={formData.phone} onChange={handleInputChange} placeholder="Enter 10-digit number" className="w-full p-2 text-sm rounded-lg focus:outline-none" />
              </div>
              {errors.phone && <p className="text-red-500 text-xs mt-1">{errors.phone}</p>}
            </div>
            <div>
              <label className="text-xs font-medium text-gray-600">Email ID</label>
              <input type="text" name="email" value={formData.email} onChange={handleInputChange} placeholder="example@hello.com" className="w-full p-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#ff5010] focus:border-transparent transition-all duration-200" />
              {errors.email && <p className="text-red-500 text-xs mt-1">{errors.email}</p>}
            </div>
            <div>
              <label className="text-xs font-medium text-gray-600">Source At</label>
              <select name="source" value={formData.source} onChange={handleInputChange} className="w-full p-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#ff5010] focus:border-transparent transition-all duration-200">
            ...
            <div>
              ...
              <DropdownButton
                ...
                items={[
                  ...
                  { label: "Agiaon", onClick: () => setFormData((prev) => ({ ...prev, block: "Agiaon" })) },
                  ...
                  { label: "Charpokhari", onClick: () => setFormData((prev) => ({ ...prev, block: "Charpokhari" })) },
                  { label: "Garhani", onClick: () => setFormData((prev) => ({ ...prev, block: "Garhani" })) },
                  { label: "Jagdishpur", onClick: () => setFormData((prev) => ({ ...prev, block: "Jagdishpur" })) },
                  { label: "Koilwar", onClick: () => setFormData((prev) => ({ ...prev, block: "Koilwar" })) },
                  { label: "Piro", onClick: () => setFormData((prev) => ({ ...prev, block: "Piro" })) },
                  ...
                  { label: "Tarari", onClick: () => setFormData((prev) => ({ ...prev, block: "Tarari" })) },
                  { label: "Udwant Nagar", onClick: () => setFormData((prev) => ({ ...prev, block: "Udwant Nagar" })) },
                ]}
              />
            ...
            <div className="col-span-2">
              ...
              <input type="text" name="subject" value={formData.subject} onChange={handleInputChange} placeholder="Enter subject" className="w-full p-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#ff5010] focus:border-transparent transition-all duration-200" />
          ...
          <div className="pt-4 pb-2">
            ...
            >
              <div className="flex flex-col items-center gap-2">
                <svg className={`w-10 h-10 ${isDragging ? "text-[#ff5010]" : "text-gray-400"} transition-colors duration-200`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
                ...
                <label className="relative">
                  <input name="attachment" onChange={handleInputChange} type="file" accept="application/pdf" hidden />
                ...
                {formData.attachment && (
                  <div className="flex items-center gap-2 mt-2 bg-gray-100 p-2 rounded-md w-full max-w-xs">
                    ...
                    <button type="button" onClick={handleRemoveFile} className="text-red-500 text-xs font-medium hover:underline">Remove</button>
                  </div>
                )}
          ...
          <div className="w-full flex gap-4 mt-4">
            <button onClick={onClose} type="button" className="w-full border border-gray-300 text-gray-700 px-4 py-2 rounded-lg font-semibold hover:bg-gray-100 transition-colors duration-200">
              Cancel
            </button>
            <button type="submit" className="w-full bg-[#ff5010] text-white px-4 py-2 rounded-lg font-semibold hover:bg-[#e6490f] transition-colors duration-200">
              Submit
            </button>
          </div>
        </form>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 transition-opacity duration-300 ease-in-out">
          <div className="bg-white w-[90%] max-w-lg rounded-2xl shadow-xl p-6 relative animate-scaleIn border-t-4 border-green-500">
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              <h2 className="text-xl font-bold text-gray-800 tracking-tight">Application Submitted Successfully!</h2>
                ...
                {qrCodeUrl && (
                  <div className="p-4 bg-gray-50 rounded-lg text-center">
                    ...
                    <div className="flex justify-center mb-3">
                      <img src={qrCodeUrl} alt="Application QR Code" className="border rounded-lg shadow-sm w-32 h-32" />
                    </div>
                    <button
                      onClick={() => {
              ...
              <p className="text-xs text-gray-600 text-center">Please save your Application ID for future reference.</p>
