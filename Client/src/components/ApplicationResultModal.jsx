// src/components/ApplicationResultModal.jsx
import React from "react";
import { motion, AnimatePresence } from "framer-motion";
import { useTranslation } from "react-i18next";
import {
  X, CheckCircle, XCircle, Clock, User, Calendar, FileText,
  QrCode, Download
} from "lucide-react";
...
const ApplicationResultModal = ({ isOpen, application, qrCodeUrl, onClose }) => {
  const { t } = useTranslation();

  if (!application) return null;

  const handleDownloadTimeline = () => {
    alert(`Downloading timeline for Application ID: ${application.applicationId}`);
  };
            ...
            <div className="p-5 sm:p-8">
              ...
              <div className="flex justify-between items-center border-b border-gray-200 pb-4 mb-6">
                <h2 className="text-xl sm:text-2xl font-semibold text-gray-900 font-['Montserrat']">
                  ...
                  <span className="text-green-700">{application.applicationId}</span>
                </h2>
                <motion.button
                  onClick={onClose}
              ...
              {/* Status */}
              <motion.div
                className={`p-4 sm:p-6 rounded-xl shadow-md flex flex-col sm:flex-row items-start sm:items-center gap-4 ${getStatusStyle(application.status).bg}`}
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
              >
                {getStatusStyle(application.status).icon}
                <div className="flex-1">
                  <div className="flex flex-wrap items-center gap-2">
                    <h3 className={`text-lg sm:text-xl font-semibold ${getStatusStyle(application.status).text} font-['Montserrat']`}>
                      {t("dashboard.modal.status")} {application.status}
                    </h3>
                    <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${getStatusStyle(application.status).badge}`}>
                      {application.status}
                    </span>
                  </div>
                  <p className={`text-sm ${getStatusStyle(application.status).text} mt-1 font-['Montserrat']`}>
                    {application.status === "Pending" && t("dashboard.statusMessages.pending")}
                    {application.status === "Compliance" && t("dashboard.statusMessages.compliance")}
                    {application.status === "Dismissed" && t("dashboard.statusMessages.dismissed")}
                  </p>
                  <p className="text-xs text-gray-500 mt-1 font-['Montserrat']">
                    {t("dashboard.modal.lastUpdated")} {application.lastUpdated}
                  </p>
                </div>
              </motion.div>
              ...
              <motion.div className="mt-6 bg-white rounded-xl p-5 shadow-md border border-gray-100">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-gray-700">
                  {[
                    { label: t("dashboard.modal.id"), value: application.applicationId, icon: <FileText size={18} className="text-green-600" /> },
                    { label: t("dashboard.modal.name"), value: application.applicantName, icon: <User size={18} className="text-green-600" /> },
                    { label: t("dashboard.modal.date"), value: application.dateOfApplication, icon: <Calendar size={18} className="text-green-600" /> },
                    { label: t("dashboard.modal.subject"), value: application.subject, icon: <FileText size={18} className="text-green-600" /> },
                    { label: t("dashboard.modal.description"), value: application.description, icon: <FileText size={18} className="text-green-600" />, colSpan: true },
                  ].map((item, i) => (
                    <motion.div
                      key={i}
                      className={`flex items-start gap-2 ${item.colSpan ? "sm:col-span-2" : ""}`}
              ...
              {qrCodeUrl && (
                <motion.div className="mt-6 bg-white rounded-xl p-5 shadow-md border border-gray-100">
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-5">
                    <img src={qrCodeUrl} alt="QR" className="border rounded-lg shadow-sm w-28 h-28 sm:w-32 sm:h-32" />
                    <div className="text-center sm:text-left flex-1">
                      ...
                      <button
                        onClick={() => {
                          ...
                          a.download = `dakpad-${application.applicationId}.png`;
                          a.href = qrCodeUrl;
                          a.click();
                        }}
                ...
                <motion.div className="mt-6 bg-white rounded-xl p-5 shadow-md border border-gray-100">
                  ...
                  <div className="relative pl-7">
                    ...
                    {application.timeline.map((step, i) => {
                      ...
                      const dot = isCompleted ? "bg-green-600" : "bg-orange-500";
                      return (
                        <motion.div
                          key={i}
                          ...
                          transition={{ delay: i * 0.06 }}
                        >
                          <div className={`w-5 h-5 rounded-full ${dot} flex items-center justify-center shadow-sm border-2 border-white`}>
...
//                   </div>
//                   <h3 className="font-semibold text-gray-800 mb-1 font-['Montserrat']">{f.title}</h3>
//                   <p className="text-sm text-gray-600 font-['Montserrat']">{f.desc}</p>
//                 </motion.div>
//               ))}
//             </motion.div>
//           </div>
